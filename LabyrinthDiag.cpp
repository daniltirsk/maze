...
vector<int> Labyrinth::findNeighbours(int nodeNumber) {
    ...
    if (nodeNumber < this->width * this->height && nodeNumber > -1)
    {
        ...
        if (nodeNumber % this->width - 1 > -1) {
            if (graph[nodeNumber - 1].name != 'b')
            {
                neighbours.push_back(nodeNumber - 1);
            }
        }
        if (nodeNumber + this->width < this->height * this->width) {
            if (graph[nodeNumber + this->width].name != 'b')
            {
                neighbours.push_back(nodeNumber + this->width);
            }
            if (nodeNumber % this->width + 1 < this->width) {
                if (graph[nodeNumber + this->width + 1].name != 'b')
                {
                    neighbours.push_back(nodeNumber + this->width + 1);
                }
            }
            if (nodeNumber % this->width - 1 > -1) {
                if (graph[nodeNumber + this->width - 1].name != 'b')
...
vector<int> Labyrinth::findShortersPath() {
    ...
    while (queue.size() > 0 && this->graph[this->end].weight < 0)
    {
        ...
        for (size_t i = 0; i < neighbours.size(); i++)
        {
            ...
            {   
                if (neighbours[i] - nodeNumber != this->width && abs(neighbours[i] - nodeNumber)>1)
                {
                    this->graph[neighbours[i]].weight = this->graph[nodeNumber].weight + 1.2;
                }
                else this->graph[neighbours[i]].weight = this->graph[nodeNumber].weight + 1;
            }
        }
